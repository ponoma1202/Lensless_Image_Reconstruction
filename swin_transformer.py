...
# Taken from: https://github.com/SwinTransformer/Swin-Transformer-Semantic-Segmentation/blob/main/mmseg/models/backbones/swin_transformer.py 
# --------------------------------------------------------
...
class SwinTransformer(nn.Module):
    ...
    def init_weights(self, pretrained=None):
        ...
        if isinstance(pretrained, str):
            ...
            # TODO: implement loading from a checkpoint
...
class FNN(nn.Module):
    def __init__(self, sum_encoder_embed_dims=1920,rec_channels=3):     # swin: 128 + 256 + 512 + 1024 = 1920
...
class Decoder(nn.Module):
    def __init__(self, sum_encoder_embed_dims=1920, rec_channels=3, original_height=210, original_width=380):
...
class SwinRecon(nn.Module):
    def __init__(self, n_channels, img_size, patch_size, embed_dim, num_heads):
        super().__init__()
        self.encoder = SwinTransformer(img_size=img_size, patch_size=patch_size, in_chans=n_channels, embed_dim=embed_dim, num_heads=num_heads) # in_22k is boolean for if model is trained on imagenet 22k. If false, pretrained on imagenet 1k
